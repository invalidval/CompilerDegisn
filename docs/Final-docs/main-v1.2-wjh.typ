...
#figure(
  image("typst/assets/image-1.png", width: 100%),
  caption: [编译器数据流图]
)
...
5. *类型检查*：
   - 变量/函数使用前必须声明。
   - 同一作用域内名字不可重复定义。
   - 赋值语句左右类型兼容（允许 `integer to real` 隐式转换）。
...
#figure(
  image("typst/assets/image-2.png", width: 100%),
  caption: [编译器总体结构图]
)
...
词法分析阶段产生的 Token 由以下结构表示：

#styled-parameter-table(
  columns: (20%, 30%, 50%),
  [字段], [类型], [说明],
  [type], [TokenType], [Token 种类：Keyword/Identifier/Number/Character/Operator/Delimiter/EndOfFile/Invalid],
  [lexeme], [string], [词素文本],
  [line], [int], [行号],
  [column], [int], [列号],
)
...
编译器采用 Visitor 模式实现 AST 遍历逻辑的分离。`ASTVisitor` 定义纯虚接口，为每种节点类型提供 `visit()` 方法。`SemanticAnnotator` 和 `CodeGenerator` 均实现该接口，分别实现语义注解和代码生成两个正交的关注点。
...
关键字表 `g_keyword_tokens` 为 `std::unordered_map`，将小写关键字字符串映射到 Bison Token 编号，共 33 个关键字条目。
```cpp
...
本节记录语法分析模块从“可归约”到“可通过测试集验收”的实现与改错过程。为避免文档与实现脱节，本节所有规则与接口均以当前 `src/parser.y`、`src/lexer.l`、`include/ast.h` 为准。

==== 测试驱动与定位手段

语法模块的改错与完善主要依赖以下观测路径：

- 构建阶段：关注 Bison 输出中的冲突告警（尤其 `reduce/reduce conflict`）。
- 语法阶段：使用 `--parse-only` 与 `test/run_parser_tests.sh` 做快速回归，聚焦“可否稳定归约 + 是否能继续恢复”。
- 集成阶段：使用 close_set / open_set 做覆盖回归，确认新增规则不会破坏既有语义动作与 AST 契约。
- 定位信息：`%locations + YYLLOC_DEFAULT + yyerror(near lexeme)` 组合输出行列与邻近词素，降低定位成本。

==== 关键问题与修复（摘录）

下表为最终实现阶段对齐测试集与语言特性的关键修复点摘要（均已落实到当前实现）。

#styled-parameter-table(
  columns: (27%, 33%, 40%),
  [现象/需求], [根因定位], [实现修复（以当前实现为准）],
  [空形参/空实参调用], [过程/函数声明与调用存在空参数场景，需要规则允许 `()`。], [在 `formal_parameter` 与 `procedure_call` / `factor` 中显式支持 `(` `)` 空列表分支。],
  [一元正号（UPLUS）], [表达式层需要接受 `+factor`，且不能破坏二元加法优先级。], [引入 `%right UPLUS`，并在 `factor` 中添加 `+ factor %prec UPLUS`。],
  [数组与字段的链式访问], [变量访问既可能是 `a[i][j]` 的多维下标，也可能是 `p.x.y` 的字段链，且两者可混合。], [`id_varpart` 递归收集 `[]` 与 `.`，并通过 `FieldAccessNode(base=nullptr)` 作为字段标记；最终在 `buildArrayAccessFromIndices()` 中统一折叠为 ArrayAccess / FieldAccess 访问链。],
  [单引号多字符常量], [测试集中出现 `'Not exist'`、`'--'` 等多字符单引号文本，若按字符处理会报错。], [词法中：单字符单引号返回 `CHARACTER`，多字符单引号返回 `STRING`；语法中 `const_value/factor` 同时接受 `STRING`。],
  [行注释兼容], [部分用例包含 `// ...` 行注释，若未跳过会产生非法字符或错误 token。], [词法中加入 `//` 行注释规则并丢弃至行末，保证与 `{}` 注释并存。],
  [RR 冲突治理], [复合语句内部存在“可空语句 + 可空语句列表”的叠加，导致同一语境多规则可空触发 RR。], [引入 `statement_list_opt` 与 `nonempty_statement` 分层：`statement` 仍可空，但 `statement_list` 从非空语句起步；同时保留 `statement_list ';' error` 作为恢复同步点。],
)
...
```cpp
// src/parser.y: buildArrayAccessFromIndices
// FieldAccessNode(base=nullptr) 作为字段标记，最终折叠为访问链。
...
其中 `test/run_parser_tests.sh` 提供 valid/invalid 双集合的快速判定；close_set/open_set 用于覆盖回归与边界场景校验。

相关文档（可追溯）：

- `docs/Wangjiahan-docs/语法分析模块详细设计-20260322.md`
- `docs/Wangjiahan-docs/AST契约与文法映射.md`
- `docs/Wangjiahan-docs/语法完成度检验报告-20260322.md`
...
*胡航宾（词法分析）*：负责 Flex 词法规则的全部实现，包括 Token 识别、注释处理、位置跟踪和词法错误检测。深入理解了正则表达式在词法分析中的应用、Flex 高级特性（如 Start Condition），以及位置跟踪的实现细节。
