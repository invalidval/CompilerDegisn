#import "typst/lib.typ": experiment-report, styled-parameter-table
...
#styled-parameter-table(
  [角色], [姓名], [职责],
  [语法分析], [王嘉晗], [AST 契约定义、Bison 建树动作、解析结果导出、模块联调],
  [组长/语义分析], [张宸宇], [符号表实现、AST 语义注解、类型检查、语义错误处理、架构设计],
  [词法分析], [胡航宾], [Flex 词法规则、Token 产出、行列号维护、词法错误检测],
...
2. *关键字*：共 31 个保留字，包括 `program`、`begin`、`end`、`const`、`var`、`type`、`record`、`procedure`、`function`、`if`、`then`、`else`、`while`、`do`、`for`、`to`、`downto`、`break`、`read`、`write`、`array`、`of`、`integer`、`real`、`boolean`、`char`、`not`、`and`、`or`、`div`、`mod`。
...
抽象语法树是整个编译器的核心数据结构，定义了统一的 `ASTNode` 基类及 22 种派生节点类型：
...
关键字表 `g_keyword_tokens` 为 `std::unordered_map`，将小写关键字字符串映射到 Bison Token 编号，共 31 个关键字条目。
...
- *关键字 Token*：`PROGRAM`、`CONST`、`VAR`、`TYPE`、`RECORD`、`INTEGER`、`REAL`、`BOOLEAN`、`CHAR` 等共 31 个。
...
`ASTVisitor` 是纯虚基类，为 22 种节点类型各定义了一个 `visit()` 方法。这使编译器能够在不修改 AST 节点类的情况下添加新的遍历逻辑。
