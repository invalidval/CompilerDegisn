...
作为本课设的亮点之一，我们基于Python设计了终端用户界面，直观展示编译过程的主要信息。

在主界面选择文件、参数等：
#image("typst/assets/image-3.png")

运行后，显示编译过程中INFO级别的日志：
#image("typst/assets/image-5.png")

也支持显示更详细的DEBUG级别日志，包括变量注册等信息：
#image("typst/assets/image-4.png")

=== IDE

在终端用户界面的基础上，我们还设计了支持Pascal-S的终端IDE。用户可在此用Pascal-S编程，可调用Pascc直接编译出C语言文件，并可进一步将C程序编译成可执行文件并运行。

IDE界面如下图，黄色区域为代码编辑区，支持语法高亮；下方为功能键提示；右侧为输出区；右下角提示光标行列号：
#image("/assets/image-10.png")

F2保存，F5编译，此时编译失败（符号x重定义），支持错误高亮：
#image("/assets/image-7.png")


将第二个x修改成y后，重新保存并编译，显示编译成功：
...
public:
    ...
    // 在当前作用域插入一个符号条目，若当前作用域已有同名符号则返回 false
    bool insert(SymbolEntry entry);

    // 返回当前作用域的嵌套深度（最外层为 0）
    int currentScopeLevel() const;

    // 从当前作用域向外逐层查找符号，返回找到的符号指针，未找到返回 nullptr
...
struct ParamInfo {
    ...
    bool isVarParam = false; // 默认不是var引用参数
};
```
...
以语句 `x := 3 + a * 2`（假设 `x`、`a` 已声明为 `integer`）为例，其 AST 结构及语义分析的深度优先遍历顺序如下图所示（边上的 ①-⑥ 为遍历次序）：

#figure(
  image("typst/assets/image.png",width: 70%),
  caption: "x := 3 + a * 2的AST结构示意"
)

各步骤注解过程说明：

- *① `annotateNode(lhs)` → `annotateIdentifier(x)`*：查找符号表 `lookup('x')` → 绑定 `symbolEntry=Var(x,Integer)`，设置 `isLValue=true`。
- *② `annotateValueNode(rhs)` → `annotateBinaryExpr(+)`*：进入二元表达式，对左右操作数递归调用 `annotateValueNode`。
- *③ `annotateLiteral(3)`*：纯数字字面量 → `dataType=Integer`。返回到 ②。
- *④ `annotateValueNode(rhs_of_+)` → `annotateBinaryExpr(*)`*：嵌套乘法表达式，继续递归左右操作数。
- *⑤ `annotateIdentifier(a)`*：查找符号表 → `dataType=Integer`。返回到 ④。
- *⑥ `annotateLiteral(2)`*：纯数字 → `dataType=Integer`。返回到 ④。
- *回溯到 ④*：检查 `isNumeric(Integer)∧isNumeric(Integer)` → 通过，`dataType=Integer`。返回到 ②。
- *回溯到 ②*：检查 `isNumeric(Integer)∧isNumeric(Integer)` → 通过，`dataType=Integer`。返回到 AssignStmt。
- *回溯到 AssignStmt*：调用 `isAssignable(lhs=Integer, rhs=Integer)` → `lhs==rhs`，通过，注解完成。

遍历整体遵循*深度优先、自底向上*的求值顺序：叶子节点先注解类型，父节点在子节点全部返回后再执行类型检查并推导自身类型。
...
`annotateUnaryExpr`：`not` 要求操作数为 `Boolean` 或 `Integer`，一元 `-` 要求操作数为 `numeric`。

以下为表达式类型检查的核心代码逻辑：

```cpp
void SemanticAnnotator::annotateBinaryExpr(BinaryExprNode* node) {
    DataType lt = inferType(lhs), rt = inferType(rhs);
    if (op == "+" || op == "-" || op == "*") {
        if (!isNumericType(lt) || !isNumericType(rt))
            report error "Arithmetic operator requires numeric operands";
        node->dataType = (lt == Real || rt == Real) ? Real : Integer;
    } else if (op == "div" || op == "mod") {
        if (!isIntegerType(lt) || !isIntegerType(rt))
            report error "div/mod operator requires integer operands";
        node->dataType = Integer;
    } else if (op == "and" || op == "or") {
        if (!isBooleanType(lt) || !isBooleanType(rt))
            report error "Logical operator requires boolean operands";
        node->dataType = Boolean;
    }
    // 关系运算符：比较类型一致性，结果返回 Boolean
}
```
...
void SemanticAnnotator::annotateIdentifier(IdentifierNode* node) {
    ...
    if (entry == nullptr) {
        report error "Undefined identifier: " + node->identifier;
        node->dataType = Unknown;
        return;
    }
    node->symbolEntry = entry;
    node->dataType = entry->type;
    node->isLValue = (entry->kind == Variable || entry->kind == Parameter);
}
```
...
void SemanticAnnotator::checkCallArguments(ProcCallNode* node, const SymbolEntry* callee) {
    if (callee->name == "read") {
        for each arg: if (!isLValue(arg))
            report error "read expects assignable variable";
        return;
    }
    if (expected != actual)  // 参数个数检查
        report error "Argument count mismatch: expected X, got Y";
    for each arg:
        if (!isAssignable(param.type, actualType))  // 类型兼容
            report error "Argument type mismatch";
        if (param.isVarParam && !isLValue(arg))      // var 参数左值检查
            report error "var parameter requires assignable argument";
}
```
...
类型兼容性由 `isAssignable()` 判定——允许 `integer → real` 的拓宽转换，其余类型要求完全匹配：

```cpp
bool SemanticAnnotator::isAssignable(DataType lhs, DataType rhs) const {
    if (lhs == Unknown || rhs == Unknown) return false;
    if (lhs == Real && rhs == Integer) return true;  // 拓宽转换
    return lhs == rhs;
}
```
...
void SemanticAnnotator::annotateBreakStmt(BreakStmtNode* node) {
    if (loopDepth_ <= 0)
        report error "break can only be used inside while/for loop";
}

void SemanticAnnotator::annotateWhileStmt(WhileStmtNode* node) {
    if (!isBooleanType(inferType(cond)))
        report error "while condition must be boolean";
    loopDepth_++;
    annotateNode(body);
    loopDepth_--;
}
...
实际实现中，`tryEvalIntConst` 递归遍历字面量、一元表达式和二元表达式进行编译期求值，对 `div`/`mod` 额外检查除零错误：

```cpp
bool SemanticAnnotator::tryEvalIntConst(ASTNode* node, int& out) const {
    if (auto* lit = dynamic_cast<LiteralNode*>(node)) {
        out = std::stoi(lit->value);  // 仅全数字字面量
        return true;
    }
    if (auto* un = dynamic_cast<UnaryExprNode*>(node)) {
        int v; if (tryEvalIntConst(un->children[0], v)) {
            out = (un->op == "-") ? -v : v; return true;
        }
    }
    if (auto* bin = dynamic_cast<BinaryExprNode*>(node)) {
        int lv, rv;
        if (tryEvalIntConst(bin->children[0], lv)
         && tryEvalIntConst(bin->children[1], rv)) {
            if (bin->op == "+") out = lv + rv;      // 加减乘
            else if (bin->op == "-") out = lv - rv;
            else if (bin->op == "*") out = lv * rv;
            else if (bin->op == "div") { if (rv == 0) return false; out = lv / rv; }
            else if (bin->op == "mod") { if (rv == 0) return false; out = lv % rv; }
            return true;
        }
    }
    return false;
...
综上所述，经过完备的测试，我们的编译器正确无误。部分典型用例及结果请见*附录B*。
...
以下为编译器实现的完整 Pascal-S 文法（含 record 扩展），采用 LALR(1) 分析，自 `parser.y` 提取。

== 程序结构

```text
program            → PROGRAM IDENTIFIER opt_program_input ';' program_body '.'
...
record_type        → RECORD field_list KW_END
...
subprogram         → PROCEDURE IDENTIFIER formal_parameter ';' subprogram_body
...
```text
compound_statement → KW_BEGIN statement_list_opt KW_END

statement_list_opt → ε
                   | ';' statement_list_opt
                   | statement_list

statement_list     → nonempty_statement
                   | statement_list ';' statement
                   | statement_list ';' error         // 恐慌模式恢复

statement          → ε
                   | nonempty_statement

nonempty_statement → variable ASSIGN expression
                   | procedure_call
                   | compound_statement
...
1. *ε* 表示空产生式。
2. 标识符（`IDENTIFIER`）不区分大小写，词法阶段统一转为小写后返回。
3. 关键字 `KW_BEGIN`/`KW_END` 分别对应 `begin`/`end`，使用带前缀的 Token 名以避免与 C++ 关键字冲突。
4. 注释支持嵌套：`{ ... }` 形式，通过 Flex Start Condition 实现。
5. 运算符优先级（从低到高）：`OR` → `AND` → 关系运算符 → `+` `-` → `*` `/` `DIV` `MOD` → `NOT` → 一元 `-`/`+`。赋值 `ASSIGN` 为右结合且优先级高于各表达式运算符。
6. 悬挂 `else` 通过 `%prec LOWER_THAN_ELSE` 解决，`else` 与最近的 `if` 绑定。
