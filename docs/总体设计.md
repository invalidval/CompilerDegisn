...
*   **映射规则**：
    ...
    *   **Record 类型映射**（扩展）：
        *   Pascal `type person = record ... end;` -> C `typedef struct { ... } person;`
        *   Pascal `p.age` -> C `p.age` (字段访问直接映射)
...
*   **核心接口**：
    ...
    *   `void CodeGenerator::visit(TypeDeclNode*)` (扩展)
    *   `void CodeGenerator::visit(FieldAccessNode*)` (扩展)
...
enum class NodeType {
    ...
    TYPE_DECL, RECORD_TYPE, FIELD_DECL, FIELD_ACCESS  // Record 类型扩展
};
...
enum DataType {
    ...
    TYPE_PROCEDURE, TYPE_FUNCTION, TYPE_RECORD  // Record 类型扩展
};
