...
type:
    ...
    {
        // 递归定义，支持 array of record
        auto* ranges = asNode($3);
        auto* elemType = asNode($6);
        $$ = static_cast<void*>(buildArrayTypeFromRanges(ranges, elemType, currentPos()));
    }
...
id_varpart:
        '.' IDENTIFIER
    {
        // 创建 FieldAccessNode 标记字段名
        auto* fieldMarker = g_astBuilder.makeFieldAccess(nullptr, std::string($2), currentPos());
...
**关键设计**：
1. 参数类型从 `basic_type` 改为 `type`，支持任意类型
2. 数组元素类型支持递归定义，允许 `array of record`
3. 使用 FieldAccessNode（base 为 nullptr）标记字段名，区分数组索引
...
void SemanticAnnotator::annotateTypeDecl(TypeDeclNode* node) {
    ...
    if (typeDefNode->nodeType == NodeType::RecordType) {
        ...
        // 使用 unordered_set 检测重复字段（O(n) 复杂度）
        std::unordered_set<std::string> fieldNames;

        // 遍历字段声明
        for (ASTNode* fieldDeclNode : recordNode->children) {
            if (auto* fieldDecl = dynamic_cast<FieldDeclNode*>(fieldDeclNode)) {
                DataType fieldType = inferTypeFromTypeNode(fieldDecl->typeNode);

                // 处理每个字段名
                for (ASTNode* idNode : fieldDecl->idList->children) {
                    std::string fieldName = toLower(idNode->identifier);

                    // 检查重复字段
                    if (!fieldNames.insert(fieldName).second) {
                        errorHandler_.report(idNode->pos.line, idNode->pos.col,
        ...
        // 插入类型到符号表
        symbolTable_.insert(entry);
    }
}
```

**关键优化**：
- 使用 `std::unordered_set<std::string>` 跟踪已定义的字段名
- `insert()` 方法返回 `pair<iterator, bool>`，其中 `.second` 表示是否成功插入
- 时间复杂度从 O(n²) 优化到 O(n)，n 为字段数量
...
void SemanticAnnotator::annotateFieldAccess(FieldAccessNode* node) {
    ...
    if (node->children.size() > 0) {
        ...
        // 情况1：直接变量访问（p.age）
        if (base->symbolEntry && !base->symbolEntry->fields.empty()) {
            fields = base->symbolEntry->fields;
        }
        // 情况2：数组元素访问（people[i].age）
        else if (auto* arrayAccess = dynamic_cast<ArrayAccessNode*>(base)) {
            if (arrayAccess->symbolEntry && !arrayAccess->symbolEntry->typeName.empty()) {
                // 查找数组元素类型的 record 定义
                const SymbolEntry* typeSym = symbolTable_.lookup(
                    arrayAccess->symbolEntry->typeName);
                if (typeSym && typeSym->kind == SymbolKind::TypeAlias) {
                    fields = typeSym->fields;
                }
            }
        }
...
**关键点**：支持两种字段访问模式：
1. 直接变量访问：从变量符号的 fields 获取字段信息
2. 数组元素访问：通过 typeName 查找类型定义，再获取字段信息
...
```cpp
DataType SemanticAnnotator::inferTypeFromTypeNode(ASTNode* typeNode) {
    // 处理标识符类型（用户定义类型）
    if (auto* typeId = dynamic_cast<IdentifierNode*>(typeNode)) {
        ...
        if (typeSym && typeSym->kind == SymbolKind::TypeAlias) {
            return typeSym->type;  // 返回 DataType::Record
        }
    }

    // 处理基本类型
    if (typeNode->nodeType == NodeType::BasicType) {
        return static_cast<BasicTypeNode*>(typeNode)->type;
    }
...
**关键点**：支持从类型标识符查找用户定义类型，使参数和数组可以使用 record 类型。
...
void SemanticAnnotator::annotateParamDecl(ParamDeclNode* node) {
    ...
    if (declaredType == DataType::Record) {
        if (auto* idTypeNode = dynamic_cast<IdentifierNode*>(typeNode)) {
            userTypeName = toLower(idTypeNode->identifier);
            typeSym = symbolTable_.lookup(userTypeName);
        }
    }

    // 为每个参数创建符号表条目
    for (ASTNode* idNode : idList->children) {
        SymbolEntry entry = SymbolEntry::makeVariable(paramName, declaredType);
        entry.isParameter = true;
        entry.isByReference = node->isByReference;

        // 复制 record 字段信息
        if (typeSym && typeSym->type == DataType::Record) {
            entry.fields = typeSym->fields;
            entry.typeName = typeSym->typeName;
        }
...
**关键点**：将 record 类型的字段信息复制到参数符号，使函数体内可以访问参数的字段。
...
void SemanticAnnotator::annotateVarDecl(VarDeclNode* node) {
    ...
    // 处理数组类型
    std::vector<std::pair<int, int>> arrayBounds;
    if (typeNode->nodeType == NodeType::ArrayType) {
        // 收集数组维度信息
        collectArrayBounds(typeNode, arrayBounds);
    }

    // 查找元素类型的 record 信息
    std::string userTypeName;
    if (declaredType == DataType::Record) {
        if (auto* idTypeNode = dynamic_cast<IdentifierNode*>(elemTypeNode)) {
            userTypeName = toLower(idTypeNode->identifier);
            const SymbolEntry* typeSym = symbolTable_.lookup(userTypeName);
            if (typeSym && typeSym->kind == SymbolKind::TypeAlias) {
                // 保存元素类型名
            }
        }
    ...
    // 创建变量符号
    SymbolEntry entry = SymbolEntry::makeVariable(varName, declaredType);
    entry.arrayBounds = arrayBounds;
    entry.typeName = userTypeName;  // 保存 record 类型名

    symbolTable_.insert(entry);
}
```

**关键点**：保存数组元素的 record 类型名，使数组访问后可以进行字段访问。
...
| 重复类型定义 | annotateTypeDecl() |
| 重复字段定义 | annotateTypeDecl() |
| 非 record 类型字段访问 | annotateFieldAccess() |
| 字段未定义 | annotateFieldAccess() |
...
public:
    ...
    // ... 其他节点类型
    void visit(TypeDeclNode* node) override;        // Record 类型
    void visit(FieldAccessNode* node) override;     // Record 类型
...
std::string CodegenUtils::mapType(DataType t) {
    switch (t) {
        ...
        case DataType::Record:  return "struct";  // Record 类型
        default:                return "int";
    }
}
```
...
void CodeGenerator::visit(FieldAccessNode* node) {
    // 生成字段访问表达式：record.field 或 array[i].field
    if (node->children.size() > 0) {
        std::string base = emitNode(node->children[0]);
...
支持两种形式的字段访问：
- 简单字段访问：`p.age` → `p.age`
- 数组元素字段访问：`people[i].age` → `people[(i) - 1].age`

#### 9.9.3 参数类型映射

在函数和过程声明中，需要正确映射 record 类型参数：

```cpp
// 在 emitFunctionDecl/emitFunctionDef 中
for (auto* id : paramIds) {
    std::string ctype;
    if (id->symbolEntry && id->symbolEntry->type == DataType::Record &&
        !id->symbolEntry->typeName.empty()) {
        ctype = id->symbolEntry->typeName;  // 使用用户定义类型名
    } else {
...
参数类型映射规则：
- 值参数：`p: person` → `person p`
- var 参数：`var p: person` → `person *p`
- 基本类型：`x: integer` → `int x`
...
procedure modifyage(var p: person; newage: integer);
begin
    p.age := newage;
end;
```
...
int getage(person p);
void modifyage(person *p, int newage);
...
Record 类型是 Pascal-S 的结构化数据类型，支持字段定义、字段访问、作为函数/过程参数以及作为数组元素。涉及以下模块：

| 模块 | 实现内容 |
|------|----------|
| 词法分析 | `type` 和 `record` 关键字 |
| 语法分析 | 类型声明、record 类型、字段声明、字段访问、参数类型支持、数组元素类型支持 |
| AST 节点 | TypeDecl、RecordType、FieldDecl、FieldAccess 节点类型 |
| 符号表 | TypeAlias 符号种类，typeName、fields、arrayBounds 字段 |
| 语义分析 | 类型声明注解、字段访问检查、用户定义类型推断、类型信息传递 |
| 代码生成 | typedef struct 生成、字段访问表达式生成、record 参数类型映射 |

**核心特性**：
1. 类型定义：支持 `type person = record ... end` 语法
2. 字段访问：支持 `p.age` 和 `arr[i].age` 等混合访问
3. 参数传递：支持值传递和引用传递（var 参数）
4. 数组元素：支持 `array[1..10] of person` 声明
5. 类型安全：完整的类型检查和字段存在性验证
6. 代码映射：Pascal record 映射为 C typedef struct
...
**V2.0 更新内容**：
- 新增 record 类型作为函数/过程参数的完整支持
- 新增 record 类型作为数组元素的完整支持
- 更新语法规则、语义分析和代码生成的实现细节